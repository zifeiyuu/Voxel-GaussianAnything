...
class pixelsplatDataset(Dataset):
    def __init__(self, cfg, split):
        ...
        choose_set = 'train' if split == "train" else 'test'
        self.dataset_folder = self.dataset_folder / choose_set
    ...
    def _load_pose_data(self):
        ...
        if self.specific_files:
            file_list = [self.dataset_folder / (file_name + '.torch') for file_name in self.specific_files]
        else:
            file_list = [self.dataset_folder / file_name for file_name in os.listdir(self.dataset_folder) if file_name.endswith('.torch')]
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.dataset_folder , 'image', str(seq_key))
        ...
        for frame_name, frame_idx in zip(frame_names, src_and_tgt_frame_idxs):
            ...
            inputs[("frame_id", 0)] = f"{self.split}+{seq_key}+index{input_frame_idx}"
