...
class GaussianDecoder(nn.Module):
    ...
    def forward(self, x, split_dimensions=[3,9,1,3,4]):
        """split the gaussian parameters
        color: 3 channel
        sh_feature: 9 channel
        opacity: 1 channel
        ...
        for i in range(self.cfg.model.gaussians_per_pixel):
            if self.cfg.model.max_sh_degree != 0:
                feat_dc_s, features_rest_s, opacity_s, scaling_s, rotation_s = outputs[i*5:(i+1)*5]
                feat_rest_list.append(features_rest_s[:, None, ...])
            else:
                feat_dc_s, opacity_s, scaling_s, rotation_s = outputs[i*4:(i+1)*4]
